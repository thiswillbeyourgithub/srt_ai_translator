...
```bash
pip install pysrt openai tqdm loguru
```
...
1. **Windowed Processing**: The script divides the SRT file into windows of N entries (default 4). This balances between:
...
2. **XML-Structured Prompts**: Each subtitle is wrapped in XML tags with timing information:
   ```xml
...
3. **Timing Context**: The start/end times help the model understand temporal relationships:
   - Rapid dialogue suggests conversation
   - Long pauses might indicate scene changes
   - Helps maintain natural flow in translations

4. **Retry Logic**: If XML parsing fails, the script:
...
5. **Incremental Saves**: After each successfully translated window:
...
- **File Not Found**: Check that the SRT file path is correct
