...
def main():
    ...
    for window_idx, window in enumerate(tqdm(windows, desc="Translating")):
        translated_window = translate_window(client, window, args.model, args.srt_context)
...
def translate_window(client, window, model, context):
    ...
    context_xml = f'<srt-context>{context}</srt-context>' if context else '<srt-context></srt-context>'
...
...
    ...
    for attempt in range(max_retries):
        try:
            ...
            translated_window = []
            for i, (original_sub, translated_text) in enumerate(zip(window, translated_texts)):
            ...
            if attempt < max_retries - 1:
                ...
                print(f"Failed to translate window after {max_retries} attempts: {e}", file=sys.stderr)
...
def parse_xml_response(response_text, expected_count):
    ...
    try:
        ...
        if len(matches) != expected_count:
            raise ValueError(f"Expected {expected_count} text elements, found {len(matches)}")
